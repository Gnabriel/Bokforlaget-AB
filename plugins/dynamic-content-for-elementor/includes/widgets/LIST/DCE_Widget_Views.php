...
class DCE_Widget_Views extends DCE_Widget_Prototype {
    ...
    protected function _register_controls() {
        ...
        $repeater_form->add_control(
            ...
            'type' => Controls_Manager::SELECT2,
            'label_block' => true,
            'options' => $post_fields + $taxonomies,
        ...
        $this->add_control(
            ...
            'type' => \Elementor\Controls_Manager::TEXT,
            'placeholder' => 'dce-basic-form-filter',
                ]
        );
        $this->add_control(
            'dce_views_where_form_class_buttons', [
                ...
                'type' => \Elementor\Controls_Manager::TEXT,
                'placeholder' => 'button-action-wrapper',
            ]
        );
        $this->add_control(
            ...
            <div class="dce-view-form-wrapper dce-view-exposed-form <?php echo $settings['dce_views_where_form_class']; ?>">
                <form id="dce-view-form-<?php echo $this->get_id(); ?>" method="get" action="">
                    <?php if ($settings['dce_views_style_form_text']) { ?>
                        <h3><?php echo $settings['dce_views_style_form_text']; ?></h3>
                    <?php } ?>
                    ...
                    foreach ($settings['dce_views_where_form'] as $key => $afield) {
                        ...
                        ?>
                        <div class="dce-view-field-wrapper <?php echo $afield['dce_views_where_form_class_wrapper']; ?> <?php echo $settings['dce_views_where_form_class_filter']; ?>">
                            <label class="dce-view-input-label <?php echo $afield['dce_views_where_form_class_label']; ?>" for="dce_view_<?php echo $afield['dce_views_where_form_field']; ?>">
                                ...
                                <?php if ($afield['dce_views_where_form_required'] && $afield['dce_views_where_form_required_label'] != 'none') { ?>
                                    <span class="dce-form-required">
                                        ...
                                        <?php if ($afield['dce_views_where_form_required_label'] == 'text') { echo $afield['dce_views_where_form_required_label_text']; } ?>
                                    </span>
                                <?php } ?>
                            </label>
                            <?php
                    ...
                    <input type="hidden" name="eid" value="<?php echo $this->get_id(); ?>">
                    <div class="dce-view-exposed-form-buttons <?php echo $settings['dce_views_where_form_class_buttons']; ?>">
                        <input class="button dce-button find <?php echo $settings['dce_views_where_form_class_button']; ?>" type="submit" value="<?php echo $settings['dce_views_style_form_submit_text']; ?>">
                        <?php if ($settings['dce_views_where_form_reset']) { ?><input class="button dce-button reset <?php echo $settings['dce_views_where_form_class_button']; ?>" type="reset" value="<?php _e('Reset'); ?>"><?php } ?>
                    </div>
                </form>
            </div>
            <?php
...
// FROM
    ...
    public function get_post_ids($awhere, $retry = null) {
        ...
        if (DCE_Helper::is_post_meta($awhere['dce_views_where_field'])) {
            ...
            if (DCE_Helper::is_plugin_active('acf') && in_array($awhere['dce_views_where_field'], array_keys(DCE_Helper::get_all_acf(false, array('repeater', 'block'))))) {
