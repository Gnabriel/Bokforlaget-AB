...
function hero_load_this_config(){var csshero_promise=jQuery.Deferred();window.csshero_theme_is_configurated=false;wcom_card=function(scope,desc){csshero_declare_item(scope,desc);csshero_declare_item(scope+' ul.products li.product > a:not(.button)',desc+' Main Link');csshero_declare_item(scope+' > h2',desc+' Area Title');csshero_declare_item(scope+' ul.products',desc+' List Area');csshero_declare_item(scope+' ul.products li.product',desc+' Item');csshero_declare_item(scope+' ul.products li.product a img',desc+' Img');csshero_declare_item(scope+' ul.products li.product h3',desc+' Title');csshero_declare_item(scope+' ul.products li.product h3 .count',desc+' Product Count');csshero_declare_item(scope+' ul.products li.product .price',desc+' Price Area');csshero_declare_item(scope+' ul.products li.product .price del',desc+' Old Price');csshero_declare_item(scope+' ul.products li.product .price ins',desc+' New Price');csshero_declare_item(scope+' ul.products li.product a.button',desc+' Buttons');csshero_declare_item(scope+' ul.products li.product .star-rating',desc+' Star Rating Area');csshero_declare_item(scope+' ul.products li.product .woocommerce-loop-product__title',desc+' Product Title');}
...
function hero_get_theme_name(){return"thegem";}
...
for(var u in paths){sele=paths[u].path;desc=paths[u].desc;try{var el=$(sele,frame);}
catch(error){console.log(sele+' is not a valid selector')}
el.each(function(){var n=$(this);csshero_apply_ediandsugg(n,sele,desc)})}
var config_loader=$('#csshero-editable-loader > span');if(need_auto_config){$('html *:not(.editable):not(.csshero-excluded-from-config)',frame).addClass('editable no-config-el');}}
function csshero_apply_custom_ediandsugg(array){$('*[custom_editableclass]',frame).removeAttr('custom_editableclass').removeAttr('custom_editablesuggestion');for(var z in array){if(array[z]['path_type']!='context_specific'){sele=array[z].path;desc=array[z].desc;if(!desc||desc==''||desc==' ')var desc=sele;try{var el=$(sele,frame);el.each(function(){if($(this).attr('custom_editableclass')){var cec=$(this).attr('custom_editableclass').split(' | ');var ces=$(this).attr('custom_editablesuggestion').split(' | ');var cec=unique(cec);var ces=unique(ces);}
...
var all=all.reverse();$.each(all,function(d){var d=$(all[d]);csshero_editables_scanner(d);})}}
...
return;}
if(!element.attr('editableclass')&&!element.is('.no-rocket *')){var edited_range=$('.csshero-currently-hovered',frame)
...
try{$(cla+':in-viewport',frame)}catch(err){if(err)return;}
var elements=$(cla+':in-viewport',frame).toArray();if(elements.length==0){var elements=$('html',frame).find(cla+':in-viewport',frame).toArray();}
var scrolled=$(frame).scrollTop()
...
var len=$(ec[0],frame).length;var is_animated='';if(hero_animations[ec[0]]){var is_animated='<span class="is_animated">Animated</span>';cl+=' probe-animated';}else{var is_animated='';}
...
for(var c in custom_els){$(c,frame).attr('editablesuggestion',custom_els[c]).attr('editableclass',c).removeClass('no-config-el').addClass('has-been-processed')}}
...
$('.hero-rc').remove()}).on('click','.focus-label .sug, .focus-label .edt',function(){var curH=$('.cur-hov-el',frame);if(!curH.is(current_el())){curH.trigger('click')}}).on('click','.chioder',function(){var curH=$('.cur-hov-el',frame);if(!curH.is(current_el())){curH.trigger('click')}
...
csshero_prepara_chiodi($('.currently-clicked',frame))}).on('click','.hero-rc [data-toggle]',function(){var t=$(this).attr('data-toggle')
$(t).toggleClass('open')
$(this).toggleClass('open')}).on('click','.hamb',function(){var t=$(this)
t.toggleClass('open');if(t.is('.open')){$('.cur-hov-el',frame).trigger('contextmenu')
...
$(frame).on('mouseenter mousemove',selector,function(e){if($('.hero-rc').length>0)return;$('.cur-hov-el',frame).removeClass('cur-hov-el')
...
$('.currently-clicked',frame).removeClass('currently-clicked')
$(this).addClass('currently-clicked')
var ecl='';for(var e in ec){var len=$(ec[e],frame).length;if(len>0&&ec[e].indexOf(':hover')==-1&&ec[e].indexOf(':active')==-1&&ec[e].indexOf(':visited')==-1){if(ct.indexOf('data-context="'+ec[e]+'"')==-1)ecl+='<li class="ico" data-context-desc="'+es[e]+'" data-context="'+ec[e]+'"><b>'+es[e]+'</b><em>'+ec[e]+'</em><span>'+len+'</span></li>';}}
var el=$('<div style="max-height:'+rch+'px;width:'+rcw+'px;top:'+t+'px;left:'+l+'px;" class="hero-rc scroller"><ul><li data-context="'+ec[0]+'" class="chioder ico">Adjust Spacings</li></ul><ul><li class="title open" data-toggle=".dd-1">Editing Context:</li><li class="dd open dd-1"><ul>'+ct+'</ul></li><li class="title open" data-toggle=".dd-2">Matching Selectors:</li><li class="dd-2 open dd"><ul>'+ecl+'</ul></li></ul></div>');el.find('li[data-context]').each(function(){var edi=$(this).attr('data-context')
...
$('.currently-clicked',frame).removeClass('currently-clicked')
$(this).addClass('currently-clicked')
$('.csshero-clicked-range',frame).removeClass('csshero-clicked-range')
...
var edited_range=$('.csshero-clicked-range',frame);edited_range=edited_range.toArray();hero_build_bottom_bar(edited_range)})})
...
function hero_generate_to_be_stored_data(){var to_be_stored={obj:{less:core.text(),animations:hero_animations},css:$('.csshero-style-render',frame).text()}
return to_be_stored;}
...
apply_style();setTimeout(function(){var val=$(current_el(),frame).css(prop)
...
get_global_vars();get_global_classes();var is_real_el=false;try{$(current_el(),frame)
...
if(atts[prop]){var val=atts[prop]}else{if(is_real_el){var val=$(current_el(),frame).eq(0).css(prop)}else{var val='none'}}
...
HEROWorker.postMessage({type:'detect_mq_edits',curel:cur,mqs:ms,val:$('.csshero-style-render',frame).text()})}
...
if(render)apply_style();if(val&&val.indexOf('!important')==-1&&val.indexOf('@')==-1&&$(current_el(),frame).length>0){setTimeout(function(){if(prop.indexOf('color')>-1){if(prop=='filter')return;if(prop.indexOf('color')>-1&&val.indexOf('rgb')==-1)return;var pse=$('<div>mu</div>');pse.css(prop,val)
if(pse.css(prop)!=$(current_el(),frame).css(prop)){$('.hero-prop[data-prop="'+prop+'"] .i-imp').trigger('click')}}},500)}}
...
var stored_data=$('#csshero-data-from-db').text();if(stored_data&&stored_data!=''){var saved_obj=JSON.parse(stored_data);if(saved_obj.less)saved_less=saved_obj.less
if(saved_obj.animations)hero_animations=saved_obj.animations;}
...
buttons+='</div>'}
var compact_class=defaults[d][dd].compact?'compact':'';var title=defaults[d][dd].custom_label?defaults[d][dd].custom_label:dd;var set_to_none=defaults[d][dd].set_none?'<span class="set-to-none ico"></span>':'';if(dd.indexOf('__')==-1){el+='<div class="hero-prop '+compact_class+' '+is_adv+' '+buttons_class+' '+ddown_class+'" data-prop="'+dd+'"><div class="prop-head"><b>'+title+'</b>'+prop_actions+'</div>'+buttons+'<div class="hero-val-wrap">'+slider+var_render+'<div class="hero-val-inner"><div class="hero-val-mask"></div><input type="text" spellcheck="false" class="hero-val" data-prop="'+dd+'" /></div>'+set_to_none+picker+numeric+ddown+'</div></div>';}else{if(dd=='__notice'){el+='<div class="hideable hero-props-notice"><span class="ico"></span>'+defaults[d][dd]+'</div>'}}}
...
hero_animations=lss.obj.animations;window.frames['csshero-iframe-main-page'].csshero_initialize_animations(hero_animations);if(!$('.wysi-inner-tab[data-tab="animations"]').is('.hid'))csshero_load_animations()
core.html(_lss)
...
code_editor=$('#hero-code-editor');editor=$('#hero-editor');frame=window.frames['csshero-iframe-main-page'].document;cssheroframe=frame;body=$('#csshero-body').addClass('hero-loaded');csshero_custom_carrier='';framewrap=$('#csshero-inner-frame-wrap');bottom_bar=$('.hero-bottom-bar-nav')
setup_global_classes();hero_store_history_step('saved')
var sen="<style>@import url('https://fonts.googleapis.com/css?family=";var fwrap=$('.hero-prop[data-prop="font-family"] .hero-dropdown-wrap');fwrap.find('li[data-val]').each(function(){var val=$(this).attr('data-val')
...
function hero_get_edited_els(){mark_code_in_core('bananas')
var gu=hell_checker.match(/£(.*?)§/g);for(var g in gu){var fum=gu[g].substring(1,gu[g].length-1);var zu=0;var sel='';try{$(fum,frame)
sel+=fum;zu+=$(fum,frame).length;}catch(err){}
sel=sel.trim()
if(sel&&sel.length>2&&zu>0){hero_edited_els.push(sel)
var ec=$(fum,frame).attr('editableclass');var es=$(fum,frame).attr('editablesuggestion');if(!ec)var ec='';if(ec.indexOf('| '+sel)>-1||ec.indexOf(sel+' |')>-1)return;var final_ec=(!ec)?sel:ec+' | '+sel;var final_es=(!es)?sel:es+' | '+sel;$(fum,frame).attr('editableclass',final_ec).addClass('editable')
$(fum,frame).attr('editablesuggestion',final_es)}}}
...
if($(clean_c,frame).prop("tagName")!='A')states=['hover']
...
if($('.scroll-anchor',frame).length>0){$('iframe').contents().scrollTop($('.scroll-anchor',frame).offset().top-200);}
...
var b=$('body',frame);var b_classes=b.attr('class').split(' ');var current={}
...
var contexts='';if(!$.isEmptyObject(context_obj)){for(var c in context_obj){var cont=context_obj[c];if(c=='normally'){}else{var oc_s=oc.split(' ');var oc_ok=[];for(var s in oc_s){var el=$(oc_s[s],frame);if(!el.is('body')&&!el.is('html')){oc_ok.push(oc_s[s]);}}
...
function hero_grab_page_colors(){return;var css=[];for(var sheeti=0;sheeti<frame.styleSheets.length;sheeti++){var sheet=frame.styleSheets[sheeti];var rules=('cssRules'in sheet)?sheet.cssRules:sheet.rules;for(var rulei=0;rulei<rules.length;rulei++){var rule=rules[rulei];if('cssText'in rule)
...
var fonts=gfonts[g].replace(/'/g,'').replace(/"/g,'').split(')')[0].split('=')[1].split('|');for(var f in fonts){var f_check=fonts[f].split('+').join(' ');if(f_check!=fonts[f]){var limit=2}else{var limit=3}
if(f_check&&code.split(f_check).length<limit){code=code.split(fonts[f]+'|').join('')
...
var el=$('.editable',frame);el.each(function(){var cl='';if(!$(this).attr('editableclass'))return;var t_c=$(this).attr('editableclass').split(' | ')[0]
...
hero_setup_scrollers();}
function hero_check_edits_from_v3(){var v4_data=hero_generate_to_be_stored_data().obj;if(!v4_data.less&&$.isEmptyObject(v4_data.animations)){var base_url=$('#csshero-body').attr('data-site-base-url');var v3_load_url=base_url+'?csshero_action=get_vintage_array';$.ajax({url:v3_load_url,success:function(data){if(data){var data=JSON.parse(data)
var v3_code='';if($.isEmptyObject(data))return;hero_animations=data.animations;for(var v in data.csshero_variables){v3_code+=v+':'+data.csshero_variables[v].var_prop+';\n';}
...
$('.csshero-style-render',frame).remove()
$('body',frame).append('<style class="csshero-style-render">'+imports+e.data.val+'</style>');hero_grab_page_colors();$('#hero-save').text('SAVE').removeClass('err')
...
$('.chioding',frame).each(function(){var os=$(this).attr('ory_style')
$(this).removeAttr('style')
if(os)$(this).removeAttr('ory_style').attr('style',os)})
$('#hero-temp-styler',frame).remove()
...
if(type=='return_page_colors'){hero_site_colors=e.data.val;}},false);var large_screens_def='@media screen and (min-width:1025px)';var default_mqs=[{name:'tablet-landscape',value:'@media screen and (max-width:1024px)'},{name:'tablet-portrait',value:'@media screen and (max-width:768px)'},{name:'mobile-landscape',value:'@media screen and (max-width:568px)'},{name:'mobile-portrait',value:'@media screen and (max-width:320px)'}]
...
$('#hero-temp-styler',frame).text(current_el()+'{transform:'+hero_push_transforms()+';}')},start:function(){$('body',frame).append('<style id="hero-temp-styler"></style>')
...
$('#hero-temp-styler',frame).text(current_el()+'{transform:'+hero_push_transforms()+';}')},start:function(){$('body',frame).append('<style id="hero-temp-styler"></style>')},stop:function(){hero_push_transforms(true);}})});})
...
$('check-chiodo',frame).each(function(){if($(this).attr('ory_style')){$(this).attr('style',$(this).attr('ory_style'))}else{$(this).removeAttr('style')}})
...
var el=$('.currently-clicked',frame)
...
body.append('<div data-ori="'+ori+'" class="hero-tip" style="'+posT+';'+posL+'"><div class="hero-tip-inner">'+tip+'</div></div>')}).on('mouseleave','[data-tip]',function(){$('.hero-tip').remove();})
...
$(window).on('frameready',function(){window.frames['csshero-iframe-main-page'].csshero_initialize_animations(hero_animations);})
...
hero_setup_scrollers();}
function csshero_apply_animation(el,animations_obj){var elis=$(el,frame);if(animations_obj&&animations_obj['data-aos']){for(var v in animations_obj){elis.attr(v,animations_obj[v])}
...
var opz={color:startcolor,preferredFormat:'rgb',showAlpha:true,showInput:true,allowEmpty:false,showInitial:true,containerClassName:customclass,showPalette:true,selectionPalette:["rgba(0,0,0,0)"],palette:[['#1abc9c','#2ecc71','#3498db','#9b59b6','#34495e','#16a085','#27ae60','#2980b9','#8e44ad','#2c3e50','#f1c40f','#e67e22','#e74c3c','#ecf0f1','#95a5a6','#f39c12','#d35400','#c0392b','#bdc3c7','#7f8c8d'],['#f44336','#E91E63','#9C27B0','#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4','#009688','#4CAF50','#8BC34A','#CDDC39','#FFEB3B','#FFC107','#FF9800','#FF5722','#795548','#9E9E9E','#607D8B','#263238'],],show:function(){$('body',frame).append('<style id="hero-temp-styler"></style>')
body.append('<div class="hero-picker-mask"></div>')},move:function(color){color=color.toRgbString();if(t.is('.hero-val-wrap *'))$('#hero-temp-styler',frame)[0].textContent=current_el()+'{'+prop+':'+color+'!important;}';},hide:function(){$('.hero-picker-mask').remove();$('.hero-picker.cur').removeClass('cur')},change:function(color){color=color.toRgbString();if(t.is('.csshero-shadow-builder *')){t.closest('.sha-node').attr('data-sha-val',color)
...
$(document).on('mouseup','.hero-picker',function(){setTimeout(function(){var cp=$('.codemirror-colorpicker');cp.append('<div class="hero-picker-footer"><span class="close-picker-dialog">Cancel</span><span class="submit-picker-dialog">OK</span></div>');},100)}).on('click','.hero-picker-footer > span',function(){if($(this).is('.submit-picker-dialog')){$('.hero-picker-mask').trigger('mouseup')}else{$('#hero-temp-styler',frame).remove();}
...
var ColorPicker=CodeMirrorColorPicker.ColorPicker;var color=(bg=='transparent'||bg=='rgba(0, 0, 0, 0)')?'white':bg;$('body',frame).append('<style id="hero-temp-styler"></style>')
body.append('<div class="hero-picker-mask"></div>')
var colorpicker=new ColorPicker({colorSets:[{name:'Flat',colors:['#1abc9c','#2ecc71','#3498db','#9b59b6','#34495e','#16a085','#27ae60','#2980b9','#8e44ad','#2c3e50','#f1c40f','#e67e22','#e74c3c','#ecf0f1','#95a5a6','#f39c12','#d35400','#c0392b','#bdc3c7','#7f8c8d']},{name:'Material',colors:['#f44336','#E91E63','#9C27B0','#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4','#009688','#4CAF50','#8BC34A','#CDDC39','#FFEB3B','#FFC107','#FF9800','#FF5722','#795548','#9E9E9E','#607D8B','#263238']},]})
...
if(t.is('.hero-val-wrap *'))$('#hero-temp-styler',frame)[0].textContent=current_el()+'{'+prop+':'+color+'!important;}';})}).on('mouseup','.hero-picker-mask',function(e){var t=$('.hero-picker[color-reference]');var color=t.attr('color-reference');if(t.is('.csshero-shadow-builder *')){t.closest('.sha-node').attr('data-sha-val',color)
t.css('background',color)
t.closest('.csshero-shadow-builder').trigger('updatesha')}
if(t.is('.gradient-picker-inner')){hero_send_color_to_gradient_builder(color)}
if(t.is('.hero-val-wrap *')){hero_update_widget($('.hero-picker.cur').closest('.hero-prop'),color,true)
...
console.log(fd)
$.post({url:csshero_admin_ajax_url,csshero_saving_nonce_field:nonce,data:fd,processData:false,contentType:false,dataType:'text',},function(response){csshero_load_images($('.hero-img-mode-wrap[data-mode="local"]'),0);$('.hero-img-mode-wrap input[type="file"]').val('')});}).on('click','.csshero-media-img',function(){var url=$(this).attr('data-url')
...
var url=ping+'/?client_id='+unsplash_app_id;$.ajax({url:url,success:function(data){}});$(this).closest('.oe.open').trigger('click')}).on('click','.csshero-unsplash-pagination > span',function(){var loader=$('.csshero-unsplash-loader');var current_page=parseInt(loader.attr('data-page'));if($(this).is('.prev')){var next_page=current_page-1;}else{var next_page=current_page+1;}
...
csshero_event_logger('snapshot','saves_snapshot')}).on('click','.hero-delete-snapshot',function(e){e.stopPropagation();if(confirm('Do you want to permanently delete this Snapshot?')){var base_url=$('#csshero-body').attr('data-site-base-url');var step=$(this).closest('.csshero-snapshot');var id=step.attr('data-csshero-step-id')
$.ajax({url:base_url+'?csshero_action=delete_snapshot&step_id='+id,success:function(data){step.fadeOut(300,function(){$(this).remove();})}});}});function hero_load_snapshots(){var base_url=$('#csshero-body').attr('data-site-base-url');var snapshots_url=base_url+'?csshero_action=list_saved_snapshots&snapshot_type=preset';var stepper=$('#hero-snapshots').empty();$.ajax({url:snapshots_url,success:function(data){if(data){var steps=$(data).find('.hero-step').clone().addClass('csshero-snapshot').append('<span class="hero-delete-snapshot ico"></span>')
stepper.append(steps)}}});}
var mirror,inspector_mirror,core,frame,wysi,code_editor,editor,body,csshero_custom_carrier,framewrap,choices,hero_root,bottom_bar,selector_bar,hero_built_in_libs,hell_checker,hero_site_colors,hero_animations,cssheroframe;var hero_edited_els=[];var live_paths={}
...
$(document).ready(function(){var check_interval=setInterval(time_checker,100);function time_checker(){load_timer+=1;if($('body').hasClass('hero-loaded')||load_timer==300){stop_check_interval()}}
...
selector_bar=$('#hero-selectors');hero_root=$('body').attr('data-plugin-root');initialize_hero();$('iframe').on('load',function(){initialize_onload();$(this).contents().scroll(function(){$('.focus-probe').remove()});mirror.on('change',hero_throttle(function(e){if(mirror.state.focused){var val=grab_between(e.getValue(),'{','}');core.find('span.hero-inner:not(:has(.hero-inner))').html('\n'+val+'\n');apply_style()}},500))
...
$('body',frame).append('<style>*{user-select:none!important;}</style>')
$('#csshero-main-stylesheet-css',frame).remove();$(window).trigger('frameready')});}).on('click','#hero-mqs span',function(){hero_load_mq_view($(this).attr('data-mq'),true);}).on('mousedown','#hero-editor:not(.inspi) .inspector-trigger',function(){body.addClass('inspector-on');hero_set_layout_prefs()
...
hero_store_history_step()
hero_animations={}
window.frames['csshero-iframe-main-page'].csshero_initialize_animations(hero_animations);csshero_load_animations();setTimeout(function(){hero_click_wrapper(current_el())},300)}}).on('click','.hero-prop-actions span',function(){var t=$(this);var input=t.closest('.hero-prop').find('.hero-val')
var prop=t.closest('.hero-prop').attr('data-prop')
...
var filj=fils.join(' ');if(e.type=='mousedown'){$('#hero-temp-styler',frame).remove();$('body',frame).append('<style id="hero-temp-styler"></style>')}
if(e.type=='input'){$('#hero-temp-styler',frame)[0].textContent=current_el()+'{filter:'+filj+'!important;}';$(this).closest('.hero-prop').find('.hero-val').val(filj)}
...
if(e.type=='mousedown'){$('body',frame).append('<style id="hero-temp-styler"></style>')}
if(e.type=='input'){$('#hero-temp-styler',frame)[0].textContent=current_el()+'{'+prop+':'+val+'!important;}';$(this).closest('.hero-prop').find('.hero-val').val(val)}
...
var c=hero_update_selector_bar($(sel,frame));hero_click_wrapper(c[0])}).on('click','.hero-load-vars',function(e){e.stopPropagation();var vl=$(this).find('.var-loader-inner');var val=$(this).closest('.hero-prop').find('.hero-val').val();vl.empty().append(hero_load_variables_panel())
...
$('#hero-editor').removeAttr('style').css('width',w).draggable('destroy')}}).on('click triggered_click','.hero-prop-actions .i-des',function(e){var t=$(this);var wid=t.closest('.hero-prop')
...
if(!editor.hasClass('max-wysiwyg'))hero_refresh_mirrors();hero_set_layout_prefs()}).on('click','.navigate',function(){body.toggleClass('enable-navigation')}).on('mouseenter mousemove click','.tree-selector-wrap',function(e){var c=$(this).attr('data-c');if(e.type=='click'&&$(c,frame).length>0){$(c,frame).eq(0).trigger('click')}else{csshero_zoom_on_element($(c,frame))}}).on('click','.close-service',function(){$('.hero-service.open').removeClass('open')}).on('click','*[data-service]',function(){var service=$(this).attr('data-service')
...
if(!wra.is('.loaded'))return;get_global_classes()}).on('click','#hero-inspector-bottom-actions span',function(){var mini=false;if($(this).is('.exp-less')){var dest=core;}else{if($(this).is('.exp-mini'))mini=true;var dest=$('.csshero-style-render',frame)}
var str=dest.text()
...
csshero_zoom_on_element($(el,frame));}).on('mouseup','.sp-cancel',function(){$('#hero-temp-styler',frame).remove()}).on('click','.hero-closer',function(){var t=$(this);if(t.is('.hero-extra *')){t.closest('.oe').trigger('click')
return;}
...
if(val&&val!=' '&&val!='.'&&val!='#'&&val!=':'){var el=$(val,frame);cnt.html('<b>'+el.length+'</b> on this page')}else{cnt.empty();}})
