...
class Elementskit_Widget_Testimonial extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ekit_watermark_icon_custom_position' => 'left: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'ekit_testimonial_wartermark_enable' => 'yes',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ekit_watermark_icon_custom_position' => 'top: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'ekit_testimonial_wartermark_enable' => 'yes',
					'ekit_testimonial_wartermark_custom_position' => 'yes',
		...
		$this->add_group_control(
			...
			[
				...
				'label' => esc_html__( 'Background', 'elementskit' ),
				'types' => [ 'classic', 'gradient' ],
				'selector' => '{{WRAPPER}} .elementskit-commentor-content > i, {{WRAPPER}} .elementskit-icon-content > i,{{WRAPPER}} .elementskit-watermark-icon > i',
				'condition' => [
					'ekit_testimonial_style!' => 'style6'
				]
			]
    ...
    protected function render_raw( ) {
		...
		$wrapper_data  .= 	($is_new || $migrated) ? 'data-left_icon="'. $settings['ekit_testimonial_left_arrows']['value'] .'"' : 'data-left_icon="'. $settings['ekit_testimonial_left_arrow'] .'"';

		// new icon
		$migrated = isset( $settings['__fa4_migrated']['ekit_testimonial_right_arrows'] );
		// Check if its a new widget without previously selected icon using the old Icon control
		$is_new = empty( $settings['ekit_testimonial_right_arrow'] );
		$wrapper_data  .= 	($is_new || $migrated) ? ' data-right_icon="'. $settings['ekit_testimonial_right_arrows']['value'] .'"' : ' data-right_icon="'. $settings['ekit_testimonial_right_arrow'] .'"';
